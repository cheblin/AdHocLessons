    ...
    public static class AdHoc
    {
        ...
        public class Pack
        {
            ...
            public class Cursor : Meta.Field.CursorBase
            {
                ...
                {
                    return Encoding.UTF8.GetString(bytes, BYTE, item_len);
                }
                ...
                public char[] encode(string src, int index, char[] reuse)
                {
                    ...
                    set_item(index, utf8.GetByteCount(reuse, 0, len));
                    utf8.GetBytes(reuse, 0, len, bytes, BYTE);
                    return reuse;
                }
        ...
        public abstract class Channel
        {
            ...
            public abstract class Transmitter
            {
                ...
                public virtual void failure(string reasone)
                {
                    Debug.Print("failure: " + reasone);
                }
                    ...
                    for(var cur = b2p.cursors[b2p.level]; 0 < bytes--; BYTE++)
                    {
                        switch(b2p.state)
                        {
                            ...
                            case Flow.STATE.PACK_ID:
                                ...
                                if(meta == null)
                                {
                                    failure("Unrecognized package ID = " + b2p.Uvalue);
                                    b2p.state = Flow.STATE.STANDBY;
                                    continue;
                                }
                                cur.meta = meta;
