				...
				if (flow->mode == CRC)
					switch (cur->subitem_len) {
						case 2: flow->Uvalue = (*src & (UMAX) 0xFF) << 8;
							cur->subitem_len = 1;
							continue;
						case 1: if ((flow->Uvalue | *src) != flow->crc) host_event(__LINE__, dst, NULL, flow->cursors[0].pack, BBOX_FAILURE_RECEIVE_CRC_ERROR);
		...
		case 4: 
		{
			if (cur->field_items == 0 || item < cur->field_item_0 || cur->field_item_0 + cur->field_items <= item || (bytes[(bit = base->BIT_S - fld->field_info_bits - 1 - (item - cur->field_item_0)) >> 3] & 1 << (bit & 7)) == 0) {
				...
				if (_item < cur->field_item_0 || cur->field_items == 0) {
					...
					if (_item < cur->field_item_0 + cur->field_items) {
